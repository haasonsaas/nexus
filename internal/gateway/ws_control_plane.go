...
func (s *wsSession) handleChatSend(frame *wsFrame) error {
	...
	if err := s.control.grpc.handleSendMessage(s.ctx, stream, req); err != nil {
		_ = s.sendEvent("error", map[string]any{
			"requestId": frame.ID,
			"code":      "runtime_error",
			"message":   err.Error(),
		})
...
func (s *wsSession) sendError(id string, code string, message string) {
	_ = s.sendResponse(id, false, nil, &wsError{Code: code, Message: message})
}
...
func (s *wsSession) startTicking() {
	...
	for {
		...
		case <-s.ctx.Done():
			return
		case <-ticker.C:
			_ = s.sendEvent("tick", map[string]any{"timestamp": time.Now().UnixMilli()})
		}
	}
}
