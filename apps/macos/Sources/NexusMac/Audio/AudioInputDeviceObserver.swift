...
/// Observes audio input device changes using CoreAudio property listeners.
/// Enables microphone selection and hot-plug detection for VoiceWake.
final class AudioInputDeviceObserver {
    private let logger = Logger(subsystem: "com.nexus.mac", category: "audio.devices")
    private var isActive = false
    private var devicesListener: AudioObjectPropertyListenerBlock?
    private var defaultInputListener: AudioObjectPropertyListenerBlock?

    /// Returns the UID of the current default input device.
    ...
    /// Returns the UIDs of all alive input devices.
    ...
    /// Returns a summary string of the default input device for logging.
    ...
    /// Installs property listeners for device changes.
    /// - Parameter onChange: Callback invoked when device configuration changes.
    ...
    /// Removes property listeners and stops observing.
    func stop() {
        guard self.isActive else { return }
        self.isActive = false
        let systemObject = AudioObjectID(kAudioObjectSystemObject)
    ...
    /// Returns the UID for a given device ID.
    ...
    /// Returns the name for a given device ID.
    ...
    /// Checks if a device is currently alive (connected).
    ...
    /// Checks if a device has input channels.
    ...
    /// Logs a change to the default input device.
