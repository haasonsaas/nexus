...
// Config holds configuration for the Microsoft Teams adapter.
type Config struct {
	...
	// WebhookURL is the public URL for receiving change notifications
	// If empty, polling mode is used
	WebhookURL string

	// PollInterval is the interval for polling messages when webhooks unavailable
	...
	// Microsoft Graph has various limits per endpoint
	RateLimit float64
...
func (c *Config) Validate() error {
	...
	// Microsoft Graph general rate limit is ~10,000 requests per 10 minutes per app
	// Be conservative with default
	if c.RateLimit == 0 {
		c.RateLimit = 10 // 10 requests per second
	}
...
// RequiredScopes returns the Microsoft Graph API scopes needed for Teams integration.
